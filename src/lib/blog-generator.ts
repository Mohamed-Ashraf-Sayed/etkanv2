...
أرجع JSON valid (بدون markdown code fences):
{
  ...
  "excerpt": "ملخص جذاب 150-160 حرف",
  "excerptEn": "Compelling excerpt in English",
  "tldr": "ملخص 40-60 كلمة",
  "content": "المحتوى بـ markdown",
  ...
  "imagePrompts": [
    "Hero image: detailed editorial cover, navy blue and gold scheme",
    "Inline image 1: explanatory diagram",
    "Inline image 2: practical example"
  ],
  "readingTime": 9
}`;
...
- Color palette: deep navy blue (#0B1F3F), gold (#D4AF37), white
- Clean, polished, magazine-quality composition
- Sharp details, professional lighting, depth and dimension
- Can include relevant Arabic/English text labels, icons, or annotations
...
export async function generateAndSaveArticle(topic: string): Promise<{
  success: boolean;
  slug?: string;
  error?: string;
}> {
  try {
    ...
    const article = await generateArticleData(topic);
    ...
    console.log(`Generating ${article.imagePrompts.length} images for "${finalSlug}"`);
    ...
    if (inlineImages.length > 0) {
      ...
      if (sections.length > 3 && inlineImages[0]) {
        sections.splice(2, 0, `![صورة توضيحية](${inlineImages[0]})\n\n`);
      }
      if (sections.length > 5 && inlineImages[1]) {
        sections.splice(4, 0, `![صورة توضيحية](${inlineImages[1]})\n\n`);
      }
    ...
    console.error(`✗ Failed to generate article for "${topic}":`, msg);
